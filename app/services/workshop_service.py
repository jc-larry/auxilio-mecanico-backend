...
class WorkshopService:
    ...
    async def list_all(self, page: int = 1, per_page: int = 10) -> tuple[list[Taller], int]:
        offset = (page - 1) * per_page
        query = select(Taller).order_by(Taller.id.desc())
        
        count_query = select(func.count()).select_from(Taller)
    ...
    async def create(self, data: WorkshopCreate) -> Taller:
        workshop = Taller(**data.model_dump())
